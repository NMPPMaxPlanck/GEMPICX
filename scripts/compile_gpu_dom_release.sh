...
CXX=CC cmake -D AMReX_DIR=$BUILD_DIR/amrex_install -D GEMPIC_USE_CUDA=ON -D CUDA_HOST_COMPILER=/opt/cray/pe/craype/2.7.3/bin/CC -D CMAKE_CPP_COMPILER=CC -D CMAKE_CUDA_FLAGS=-ccbin=CC -D MPI_CUDA_INCLUDE_PATH='' -D MPI_CUDA_LIBRARIES='' -D CMAKE_BUILD_TYPE=Release $SOURCE_DIRECTORY -D CMAKE_EXE_LINKER_FLAGS=-lnvToolsExt
gmake -j 16
