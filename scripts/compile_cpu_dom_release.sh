...
CXX=CC cmake -D CMAKE_BUILD_TYPE=Release -D CMAKE_INSTALL_PREFIX=../amrex_install -DAMReX_OMP=NO -D AMReX_SPACEDIM=3 -D AMReX_PARTICLES=ON -D AMReX_TINY_PROFILE=ON -D CUDA_HOST_COMPILER=/opt/cray/pe/craype/2.7.3/bin/CC -D CMAKE_CPP_COMPILER=CC -D CMAKE_LINKER=CC -D CMAKE_CUDA_FLAGS=-ccbin=CC --expt-extended-lambda $AMREX_DIRECTORY
...
CXX=CC cmake -D AMReX_DIR=$BUILD_DIR/amrex_install -D GEMPIC_USE_CUDA=OFF -D USE_OMP=OFF -D CUDA_HOST_COMPILER=/opt/cray/pe/craype/2.7.3/bin/CC -D CMAKE_CPP_COMPILER=CC -D CMAKE_CUDA_FLAGS=-ccbin=CC -D MPI_CUDA_INCLUDE_PATH='' -D MPI_CUDA_LIBRARIES='' -D CMAKE_BUILD_TYPE=Release $SOURCE_DIRECTORY
