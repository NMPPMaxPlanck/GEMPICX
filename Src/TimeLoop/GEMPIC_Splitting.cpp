...
/**
 * @brief Pushes position and velocity of particle in an analytical constant magnetic field
 *
 * Strang splitting (Hamiltonian split into directions), each step solved analytically by use of
 * known primitive of B.
 *
 ...
 *
 */
AMREX_GPU_HOST_DEVICE void Gempic::TimeLoop::push_particle_constant_b(
    amrex::GpuArray<amrex::Real, GEMPIC_SPACEDIM>& pos,
    amrex::GpuArray<amrex::Real, GEMPIC_SPACEDIM>& posOld,
    amrex::GpuArray<amrex::Real, 3>& vel,
    amrex::Array<amrex::ParserExecutor<GEMPIC_SPACEDIM + 1>, 3> funcBPrimX,
    amrex::Array<amrex::ParserExecutor<GEMPIC_SPACEDIM + 1>, 3> funcBPrimY,
    amrex::Array<amrex::ParserExecutor<GEMPIC_SPACEDIM + 1>, 3> funcBPrimZ,
    amrex::Real const chargeMass,
    amrex::Real const dt)
{
    pos[xDir] += 0.5 * dt * vel[xDir];
    vel[yDir] -= 0.5 * dt * chargeMass *
                 (funcBPrimX[zDir](AMREX_D_DECL(pos[xDir], pos[yDir], pos[zDir]), 0.) -
                  funcBPrimX[zDir](AMREX_D_DECL(posOld[xDir], pos[yDir], pos[zDir]), 0.));
    vel[zDir] += 0.5 * dt * chargeMass *
                 (funcBPrimX[yDir](AMREX_D_DECL(pos[xDir], pos[yDir], pos[zDir]), 0.) -
                  funcBPrimX[yDir](AMREX_D_DECL(posOld[xDir], pos[yDir], pos[zDir]), 0.));
#if GEMPIC_SPACEDIM > 1
    pos[yDir] += 0.5 * dt * vel[yDir];
    vel[xDir] += 0.5 * dt * chargeMass *
                 (funcBPrimY[zDir](AMREX_D_DECL(pos[xDir], pos[yDir], pos[zDir]), 0.) -
                  funcBPrimY[zDir](AMREX_D_DECL(pos[xDir], posOld[yDir], pos[zDir]), 0.));
    vel[zDir] -= 0.5 * dt * chargeMass *
                 (funcBPrimY[xDir](AMREX_D_DECL(pos[xDir], pos[yDir], pos[zDir]), 0.) -
                  funcBPrimY[xDir](AMREX_D_DECL(pos[xDir], posOld[yDir], pos[zDir]), 0.));
#endif
#if GEMPIC_SPACEDIM == 3
    pos[zDir] += dt * vel[zDir];
    vel[xDir] -= dt * chargeMass *
                 (funcBPrimZ[yDir](AMREX_D_DECL(pos[xDir], pos[yDir], pos[zDir]), 0.) -
                  funcBPrimZ[yDir](AMREX_D_DECL(pos[xDir], pos[yDir], posOld[zDir]), 0.));
    vel[yDir] += dt * chargeMass *
                 (funcBPrimZ[xDir](AMREX_D_DECL(pos[xDir], pos[yDir], pos[zDir]), 0.) -
                  funcBPrimZ[xDir](AMREX_D_DECL(pos[xDir], pos[yDir], posOld[zDir]), 0.));
#endif
#if GEMPIC_SPACEDIM > 1
    pos[yDir] += 0.5 * dt * vel[yDir];
    vel[xDir] += 0.5 * dt * chargeMass *
