...
int main (int argc, char *argv[])
{
    amrex::Initialize(argc, argv);
    {
        ...
        amrex::PrintToFile("test_FDHodge_degree_4_extrapolation.output") << std::endl;
        amrex::PrintToFile("test_FDHodge_degree_4_extrapolation.output")
            << GEMPIC_SPACEDIM
            << "D Finite Difference Hodge degree 4 extrapolation convergence test:" << std::endl;
        amrex::PrintToFile("test_FDHodge_degree_4_extrapolation.output") << std::endl;
        for (int i = 0; i < len; ++i)
        {
            amrex::PrintToFile("test_FDHodge_degree_4_extrapolation.output").SetPrecision(3)
                << hodges[i] + " rate: " << rate[i] << std::endl;
        }

        if (amrex::ParallelDescriptor::MyProc() == 0)
        {
            std::rename("test_FDHodge_degree_4_extrapolation.output.0",
                        "test_FDHodge_degree_4_extrapolation.output");
        }
    }
    amrex::Finalize();
}
