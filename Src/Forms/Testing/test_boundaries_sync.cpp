...
int main (int argc, char *argv[])
{
    ...
    Gempic::Io::Parameters parameters{};
    {
        ...
        const amrex::Array<std::string, 3> analyticalE = {"0.0", "0.0", "cos (x)"};
        ...
        for (int comp = 0; comp < GEMPIC_SPACEDIM; ++comp)
        {
            for (amrex::MFIter mfi(field.m_data[comp]); mfi.isValid(); ++mfi)
            {
                ...
                const amrex::GpuArray<amrex::Real, GEMPIC_SPACEDIM> dr = infra.m_dx;
                const amrex::GpuArray<amrex::Real, GEMPIC_SPACEDIM> r0 =
                    infra.m_geom
                        .ProbLoArray();  // Put ProbLo in parameters as a getter. Domainbounds...
                ...
                ParallelFor(bx,
                            [=] AMREX_GPU_DEVICE(int i, int j, int k)
                            {
                                ...
                                oneForm(i, j, k) =
                                    func[comp](AMREX_D_DECL(r[xDir], r[yDir], r[zDir]));
                            });

                // Modify MF boundaries to see whether the value is averaged
                // There are 4 values intersecting 1, 1, 1.3, 1.7 (the 5th 4,0,0 does not refer to
                // the same centeredness
        ...
        bool passed = true;
        for (amrex::MFIter mfi(field.m_data[zDir]); mfi.isValid(); ++mfi)
        {
            const amrex::Box &bx = mfi.validbox();
            const auto lo = lbound(bx);
            const auto hi = ubound(bx);
            passed = (aux == (field.m_data[zDir])[0].array()(4, 0, 0)) &&
                     (aux == field.m_data[zDir][1].array()(4, 0, 0));
        }
