...
Filter.type = Bilinear # String. Default Bilinear
\end{lstlisting}

Specific settings for the bilinear filter. See \ref{sec:BilinearFilter} for details.
\begin{lstlisting}[language=pkgconfig]
...
Specific settings for the Fourier filter. This filter is only implemented for fully periodic domains. See \ref{sec:FourierFilter} for details.
\begin{lstlisting}[language=pkgconfig]
Filter.type = Fourier

# The minimum Fourier mode number that should be kept in each direction
Filter.nMin = 0 0 0 # Integer vector of AMReX_SPACEDIM entries. No default.

# The maximum Fourier mode number that should be kept in each direction
Filter.nMax = 10 10 10 # Integer vector of AMReX_SPACEDIM entries. No default.
\end{lstlisting}
...
# Particle ParticleVariance ElecFieldEnergy MagFieldEnergy GaussError
...
For relativistic sampling, one has to be careful with the exact meaning of \verb|vMean| and \verb|vThermal|. See \ref{sec:JuettnerSampler} for details.
\begin{lstlisting}[language=pkgconfig]
Particle.{speciesName}.sampler = Juettner
Particle.{speciesName}.G0.vMean = 0 0.5 0 # Beta for Lorentz transformation. Non-zero in at most one direction.
Particle.{speciesName}.G0.vThermal = 1 1 1 # sqrt(T / mc^2). Needs to be isotropic.
Particle.{speciesName}.nMaxRejections = 100 # Integer. Default 100.
\end{lstlisting}
...
\subsection{Advanced Input File Features}
\subsubsection{Math expressions}
\begin{lstlisting}[language=pkgconfig]
ComputationalDomain.domainHi = sqrt(2/3) exp((-1)^2) sin(3+1)
ComputationalDomain.domainHi = "sqrt( 2 / 3 )" "exp( (-1) ^ 2)" "sin(3 + 1)"
ComputationalDomain.domainHi = "sqrt( 2 / 3 )"  \
							   "exp( (-1) ^ 2)" \
							   "sin(3 + 1)"
\end{lstlisting}
For lists/vectors of values, whitespaces are important. Whitespaces inside \verb|""| are okay.

\subsubsection{Variables and mathematical constants}
\begin{lstlisting}[language=pkgconfig]
factor = 2
ComputationalDomain.nCells = 16*factor 8*factor 8*factor
ComputationalDomain.nCells = "16 * factor" "8 * factor" "8 * factor"
TimeLoop.dt = 0.1/factor
\end{lstlisting}
Such variables cannot be used in expressions that are parsed like \verb|Function.Ex = "sin(x)"|.

In contrast, the constants \verb|pi| and \verb|e| can be used in mathematical expressions as well as parsed function strings. It is possible to define custom constants by using the prefix \verb|constants|
\begin{lstlisting}[language=pkgconfig]
Constants.a = 5
b = a # no prefix needed when referencing the constant
Function.Ex = "a * pi" # constants can be used in parsed expressions
\end{lstlisting}

\subsubsection{Overwriting values}
If a variable is defined multiple times, the last occurrence will override previous ones even if it appears after the variable has been referenced.
\begin{lstlisting}[language=pkgconfig]
a = 5
b = a
a = 42
\end{lstlisting}
will result in \verb|a = 42, b = 42|.

\subsubsection{Overwriting values from CLI}
\begin{lstlisting}[language=pkgconfig]
./vlasovmaxwell weibel.input factor=2
\end{lstlisting}
The values take precedence over those in the input file. One possible application are scaling studies where one can easily adjust problem size and distribution with a single parameter.

\subsubsection{Dimension dependent values}
\begin{lstlisting}[language=pkgconfig]
#if AMREX_SPACEDIM == 1
Particle.ions.nPartPerCell = 1000
#elif (AMREX_SPACEDIM == 2)
Particle.ions.nPartPerCell = 100
#else
Particle.ions.nPartPerCell = 10
#endif
\end{lstlisting}
The parantheses around the condition are optional. The available relation operators are \verb|<, >, <=, >=|. From AMReX version 26.03 there will also be support for the \verb|AMREX_USE_GPU| macro with operators \verb|#ifdef, #ifndef|.

\subsubsection{Including files}
\begin{lstlisting}[language=pkgconfig]
FILE = base.input
\end{lstlisting}
Includes a named file at the current position of the input file. The extension can be arbitary and the file location can be given relative or absolute.
