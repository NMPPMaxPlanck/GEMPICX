...
\begin{enumerate}
   ...
   \begin{itemize}
      ...
      \item Formatting: set \verb|C_Cpp.clang_format_style| to \verb|.clang-format| (the \verb|.clang-format| file is in the home of the gempic repository)
   \end{itemize}
\end{enumerate}
...
Debugging: Two configuration examples are in \verb|scripts/launch.json|
\begin{itemize}
   \item \verb|vlasov-maxwell|: simulation example with input file
   \item \verb|(lldb) Cmake|: debugs the target that is set in VSCode (without input file). Can be used to debug ctests
\end{itemize}
