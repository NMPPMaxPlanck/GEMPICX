...
\begin{itemize}
    ...
    \item (*) Todo comments should be in the Doxygen style, that is, \lstinline{/**}/\lstinline{/*!}/\lstinline{///}/\lstinline{//!} followed by \lstinline{@todo}/\lstinline{\todo}, \emph{i.e.} \lstinline{/// @todo: Don't forget to do documentation!}
...
\subsection{Assertions, Errors, and Warnings}
\begin{itemize}
    \item \lstinline{GEMPIC_ALWAYS_ASSERT_WITH_MESSAGE(condition, message)} prints message to cerr and aborts if condition is violated
    \item \lstinline{GEMPIC_ERROR(message)} prints message to cerr and aborts
    \item \lstinline{amrex::Warning("Warning: " + message)} prints message to cerr with an exclamation mark appended automatically
\end{itemize}
In all three cases, \lstinline{message} is a string that can be dynamically created at runtime. Only use the AMReX macros/functions directly if needed in device code.

\section{Automatic Linting and Formatting}
To help enforce the formatting and naming conventions we use two clang extra tools: \verb|clang-format| and \verb|clang-tidy|.
