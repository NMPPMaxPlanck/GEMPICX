...
Currently a bilinear and a Fourier filter are available (See \ref{sec:FilterInput} for the controls).
...
\section{Fourier Filter}\label{sec:FourierFilter}
In contrast to local real-space filters such as bilinear smoothing, which damp all wavelengths, the Fourier band-pass filter provides explicit control over which spectral modes are retained or removed. The filter applies a rectangular band-pass mask in the frequency domain. The mask is configured by specifying its lowest and highest mode number in each direction. Due to limitations in AMReX's FFT interface, the filter is currently limited to fully periodic domains.
