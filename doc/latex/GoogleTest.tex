...
\gt{} on its own can sometimes provide more useful output than the standard \ctest{} framework. This comprises all of the unit tests, but excludes the runtest examples. If you are only interested in \gt{} tests, you can run the \verb|GtestTests| executable in the \verb|Testing| subfolder of your build folder. In this case, specific tests are run with the flag \verb|--gtest_filter=PartOfTestName*|.
...
AMReX should accept any arguments given, but it is probably better to feign this functionality using either \lstinline{amrex::ParmParse} or the \lstinline{Parameters} class to set variables (to keep options contained), and change runtime variables in \verb|TestUtils/AmrexTestEnv.H| if needed.

\subsection{Testing GPU Code}\label{sec:testing-gpu-code}
...
\subsection*{Frequently Asked Questions: GoogleTest}
\begin{itemize}
    \item \emph{\ldots why?}
    ...
    See the GPU section~\ref{sec:testing-gpu-code}.
