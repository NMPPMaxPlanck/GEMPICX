...
Controls the boundary conditions on non-periodic domains (assuming your simulation supports this option). If the simulation domain contains non-periodic boundaries, i.e., \verb|isPeriodicVector| contains one or more 0 entries, boundary conditions can be provided. Specific boundary conditions can only be assigned to named fields. For all other fields the default is used.
...
\begin{itemize}
   \item \lstinline[language=pkgconfig]{FFT}: Using the Fast Fourier Transform to solve the Poisson equation in Fourier space. Default if the boundary conditions are periodic and FFT was found during installation.
   \item \lstinline[language=pkgconfig]{Hypre}: Using the sparse solver library Hypre. Default if Hypre is available and FFT is not applicable.
   \item \lstinline[language=pkgconfig]{ConjugateGradient}: Uses our own conjugate gradient solver. Default if neither FFT nor Hypre apply.
   \item \lstinline[language=pkgconfig]{ConjugateGradientInverseHodge}: Same as ConjugateGradient, but using another conjugate gradient solver instead of the Hodge operator such that $-\tildemat{D}{\mat{H}}_2\mat{G}\arr{\phi}=\arr{\rho}$ exactly.
   \item "Amrex": Using the AMReX MLMG solver. Boundary conditions must be periodic.
\begin{lstlisting}[language=pkgconfig]
# PoissonSolver
