...
\begin{itemize}
    ...
    \item Calculate component-wise the location of each point in the grid $\bfr = \{ r_{0,x} + idx, r_{0,y} + jdy, r_{0,z} + kdz \}$, being $\bfr_0$ the reference point and $i, j, k$ the indexes of the point in the grid (we need to take into account the centeredness of the grid, for the dual grid we need to shift the point by half).
    \item Calculate the midpoint of the quadratule rule after having the position of the point in the grid: $\bfr_{comp,mid} = \bfr + \frac{d\bfr_{comp}}{2}$
    \item Compute the integral along the direction of the component in two steps looping over the $N$ abscissas, which we denominate here by $q_l$ being l and index from $1$ to $N$:
        \begin{enumerate}
        \item location of the quadrature point: $\bfr_{comp} = \bfr_{comp, mid} + q_l \frac{d\bfr_{comp}}{2}$
...
The implementation of the Gauss quadrature is similar to the one used for the 1-form. Although, there are two midpoints (one for each direction integrated) and two integrals.
