...

    {
        ...
        { //"Time Loop" scope
            ...
            for (int tStep = 0; tStep < nSteps; tStep++)
            {
                ...
                deposit_current_density<degx, degy, degz, vdim, ndata>(J, partGr, infra);
                copy(jOld, J);
                hypreQNLinearSystem.push_particles_and_correct_div_j(J, phiCorr, partGr, dt);
                check_j_change_norms(jOld, J, deRham, infra);
                copy(jOld, J);
                hypreQNLinearSystem.push_particles_and_correct_javg(J, rho, partGr);
                check_j_change_norms(jOld, J, deRham, infra);
                amrex::Print() << "\n";
                ...
                hypreQNLinearSystem.update_particle_positions_and_velocities(
                    E, B, partGr, dt, RotatePushOrder::PushvThenRotatev);

                amrex::Print() << "Obtain V(" << tStep + 1 << ") -> ";
                // Update V using E and B
                hypreQNLinearSystem.update_particle_positions_and_velocities(
                    E, B, partGr, dt, RotatePushOrder::RotatevThenPushv);
