...
void write_rho(DeRhamField<Grid::dual, Space::cell> &rho, computational_domain& infra, double time, int step) {
        // save fields
        // MultiFab Info -------------------------------------------------------------
        std::string plotfilename{"Plotfiles/" + amrex::Concatenate("rhorho", step)};
...
//    nGhost[0] = std::max(nGhost[0], degx);
//    nGhost[1] = std::max(nGhost[1], degy);
//    nGhost[2] = std::max(nGhost[2], degz);

    std::cout << "Number of ghost cells: " << nGhost[0] << std::endl;
    std::cout << "Number of ghost cells: " << nGhost[1] << std::endl;
    std::cout << "Number of ghost cells: " << nGhost[2] << std::endl;
...
//                &(pti.GetArrayOfStructs()[0]);  // get particles
                ...
                amrex::ParallelFor(np, [=] AMREX_GPU_DEVICE(long pp)
                {
                    ...
                    positionParticle[0] = positionParticle[0] + dt * velx[pp];
                    partData[pp].pos(0) = positionParticle[0];
                    positionParticle[1] = positionParticle[1] + dt * vely[pp];
