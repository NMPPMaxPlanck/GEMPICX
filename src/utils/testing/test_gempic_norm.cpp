...
void main_main ()
{
    ...
    AllPrintToFile("test_gempic_morm.tmp") << endl << "Constant case: " << endl;
    AllPrintToFile("test_gempic_morm.tmp") << "0-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 0) - C) << endl;
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(5) << "1-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 0) - C) << endl;
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(5) << "2-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 0) - C) << endl;
    AllPrintToFile("test_gempic_morm.tmp") << endl;
    ...
    AllPrintToFile("test_gempic_morm.tmp") << "Linear case: " << endl;
#if(GEMPIC_SPACEDIM == 1)
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(5) << "0-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 0) - (a*infra.Length[0])) << endl;
    double Lx = infra.Length[0];
    double norm1 = 1./2.*Lx*a;
    double norm2 = 1./3.*pow(Lx,2.)*pow(a,2.);
    AllPrintToFile("test_gempic_morm.tmp") << "1-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 1) - norm1) << endl;
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(3) << "2-norm error: " << floor(fabs(gempic_norm(&mw_yee.rho, infra, 2) - norm2)*1000) << endl;
#endif
#if (GEMPIC_SPACEDIM == 2)
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(5) << "0-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 0) - (a*infra.Length[0] + b*infra.Length[1])) << endl;
    double Lx = infra.Length[0];
    double Ly = infra.Length[1];
    double norm1 = 1./2.*(a*Lx + b*Ly);
    double norm2 = 1./6.*(2.*pow(a,2.)*pow(Lx,2.) + 3.*a*b*Lx*Ly + 2.*pow(b,2.)*pow(Ly,2.));
    AllPrintToFile("test_gempic_morm.tmp") << "1-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 1) - norm1) << endl;
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(1) << "2-norm error: " << floor(fabs(gempic_norm(&mw_yee.rho, infra, 2) - norm2)*1000) << endl;
#endif
#if (GEMPIC_SPACEDIM == 3)
    AllPrintToFile("test_gempic_morm.tmp").SetPrecision(5) << "0-norm error: " << fabs(gempic_norm(&mw_yee.rho, infra, 0) - (a*infra.Length[0] + b*infra.Length[1] + c*infra.Length[2])) << endl;
    double Lx = infra.Length[0];
    double Ly = infra.Length[1];
...
int main(int argc, char* argv[])
{
    ...
    main_main<3, 1>();
#endif
    if (ParallelDescriptor::MyProc()==0) std::rename("test_gempic_morm.tmp.0", "test_gempic_norm.output");
