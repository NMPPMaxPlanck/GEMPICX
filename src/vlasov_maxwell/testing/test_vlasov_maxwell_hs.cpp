...
void main_main ()
{
    ...
    for (int t_step=0;t_step<mw_yee.nsteps;t_step++) {
        ...
        switch (strang_order) {
        case 2:
            time_loop_hs_fem<vdim, numspec, degx, degy, degz, degmw>(infra, &mw_yee, 1.0, &part_gr, &diagn, ctest, "test_vlasov_maxwell_hs.tmp", &ofs, &profiling_timers);
            break;
        case 4:
            amrex::Real alpha = 1./(2.-pow(2.,1./3.));
            amrex::Real beta = 1. - 2.*alpha;

            time_loop_hs_fem<vdim, numspec, degx, degy, degz, degmw>(infra, &mw_yee, alpha, &part_gr, &diagn, ctest, "test_vlasov_maxwell_hs.tmp", &ofs, &profiling_timers);
            time_loop_hs_fem<vdim, numspec, degx, degy, degz, degmw>(infra, &mw_yee, beta, &part_gr, &diagn, ctest, "test_vlasov_maxwell_hs.tmp", &ofs, &profiling_timers);
            time_loop_hs_fem<vdim, numspec, degx, degy, degz, degmw>(infra, &mw_yee, alpha, &part_gr, &diagn, ctest, "test_vlasov_maxwell_hs.tmp", &ofs, &profiling_timers);
            break;
        }

        diagn.end_of_timestep(&profiling_timers, t_step, infra, &mw_yee, &part_gr, "test_vlasov_maxwell_hs.tmp", ctest);

    }

    diagn.save_all_to_textfile(mw_yee.dt, "test_vlasov_maxwell_hs.tmp");
}
