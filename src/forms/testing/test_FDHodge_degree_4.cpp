/*------------------------------------------------------------------------------
 Test Finite Difference Hodge convergence rates
  ...
  in 3D computes the max norm ||I_{3-k} H_k R_k f - 1/omega f|| at the cell mid points
...
amrex::Real test12(int n)
{
    ...
    const amrex::RealBox realBox({AMREX_D_DECL(0, 0, 0)},{AMREX_D_DECL(1, 1, 1)});
...
int main (int argc, char *argv[]) 
{
	amrex::Initialize(argc, argv); 
    {
        ...
        amrex::PrintToFile("test_FDHodge_degree_4.output") << "Finite Difference Hodge degree 4 convergence test:" << std::endl;
        amrex::PrintToFile("test_FDHodge_degree_4.output") << std::endl;
        for (int i = 0; i < len; ++i)
        {
