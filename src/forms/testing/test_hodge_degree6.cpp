...
int main (int argc, char *argv[]) 
{
	...
	const amrex::IntVect nCell = {AMREX_D_DECL(120, 120, 120)};
    const amrex::IntVect maxGridSize = {AMREX_D_DECL(120, 120, 120)};
    ...
    for (int comp = 0; comp < 3; ++comp)
    {
        for (amrex::MFIter mfi(errorE.data[comp]); mfi.isValid(); ++mfi)
        {
            ...
            {
                if ((i < lo.x + 4 || j < lo.y + 4 || k < lo.z + 4) || (i >= hi.x - 4 || j >= hi.y - 4 || k >= hi.z - 4))
                    errorMF(i, j, k) = 0.0;
                else
    ...
    for (int comp = 0; comp < 3; ++comp)
    {
        ...
        for (amrex::MFIter mfi(errorE.data[comp]); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                amrex::Print() << "(" << i << "," << j << "," << k << ") errorE: " << errorMF(i, j, k) << std::endl;
            });
        }
        ...
        for (amrex::MFIter mfi(errorH.data[comp]); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                amrex::Print() << "(" << i << "," << j << "," << k << ") errorH: " << errorMF(i, j, k) << std::endl;
            });
        }
    }
    */
    ...
    for (int comp = 0; comp < 3; ++comp)
    {
        amrex::Print() << "comp: " << comp << " errorE: " << errorE.data[comp].norm0() << std::endl;
        amrex::Print() << "comp: " << comp << " errorH: " << errorH.data[comp].norm0() << std::endl;
    }
    */
    ...
    {
        amrex::PrintToFile("test_hodge_degree6.output") << std::endl;
        amrex::PrintToFile("test_hodge_degree6.output") << true << std::endl;
    }
    else
    {
        amrex::PrintToFile("test_hodge_degree6.output") << std::endl;
        amrex::PrintToFile("test_hodge_degree6.output") << false << std::endl;
    }

    if (amrex::ParallelDescriptor::MyProc() == 0)
        std::rename("test_hodge_degree6.output.0", "test_hodge_degree6.output");
