...
int main (int argc, char *argv[])
{
    amrex::Initialize(argc, argv);
    {
        ...
        if (errorCoarse < 1e-15 || errorFine < 1e-15)
        {  // this happens..
            rate[2] = 2;
        }
        else
        ...
        amrex::PrintToFile("test_FDHodge_degree_2.output")
            << GEMPIC_SPACEDIM
            << "D Finite Difference Hodge degree 2 convergence test:" << std::endl;
        amrex::PrintToFile("test_FDHodge_degree_2.output") << std::endl;
        for (int i = 0; i < len; ++i)
        {
            amrex::PrintToFile("test_FDHodge_degree_2.output").SetPrecision(3)
