...
void
MLNodeLaplacian_FD::define (const Vector<Geometry>& a_geom,
                         const Vector<BoxArray>& a_grids,
                         const Vector<DistributionMapping>& a_dmap,
                         const LPInfo& a_info,
                         const Vector<FabFactory<FArrayBox> const*>& a_factory)
...
void
MLNodeLaplacian_FD::unimposeNeumannBC (int amrlev, MultiFab& rhs) const
{
...
void
MLNodeLaplacian_FD::setSigma (int amrlev, const MultiFab& a_sigma)
{
    MultiFab::Copy(*m_sigma[amrlev][0][0], a_sigma, 0, 0, 1, 0);
}

void
MLNodeLaplacian_FD::compDivergence (const Vector<MultiFab*>& rhs, const Vector<MultiFab*>& vel)
{
    compRHS(rhs, vel, Vector<const MultiFab*>(), Vector<MultiFab*>());
}

void
MLNodeLaplacian_FD::compRHS (const Vector<MultiFab*>& rhs, const Vector<MultiFab*>& vel,
...
void
MLNodeLaplacian_FD::updateVelocity (const Vector<MultiFab*>& vel, const Vector<MultiFab const*>& sol) const
{
...
void
MLNodeLaplacian_FD::getFluxes (const Vector<MultiFab*> & a_flux, const Vector<MultiFab*>& a_sol) const
{
#if (AMREX_SPACEDIM == 2)
    bool is_rz = m_is_rz;
#endif
...
void
MLNodeLaplacian_FD::averageDownCoeffs ()
{
...
void
MLNodeLaplacian_FD::averageDownCoeffsToCoarseAmrLevel (int flev)
{
    const int mglev = 0;
...
void
MLNodeLaplacian_FD::averageDownCoeffsSameAmrLevel (int amrlev)
{
...
void
MLNodeLaplacian_FD::FillBoundaryCoeff (MultiFab& sigma, const Geometry& geom)
{
...
void
MLNodeLaplacian_FD::fixUpResidualMask (int amrlev, iMultiFab& resmsk)
{
    if (!m_masks_built) buildMasks();
...
void
MLNodeLaplacian_FD::interpolation (int amrlev, int fmglev, MultiFab& fine, const MultiFab& crse) const
{
...
void
MLNodeLaplacian_FD::averageDownSolutionRHS (int camrlev, MultiFab& crse_sol, MultiFab& crse_rhs,
...
void
MLNodeLaplacian_FD::compSyncResidualCoarse (MultiFab& sync_resid, const MultiFab& a_phi,
...
void
MLNodeLaplacian_FD::compSyncResidualFine (MultiFab& sync_resid, const MultiFab& phi, const MultiFab& vold,
                                       const MultiFab* rhcc)
{
...
void
MLNodeLaplacian_FD::reflux (int crse_amrlev,
...
#ifdef AMREX_USE_EB
void
MLNodeLaplacian_FD::buildIntegral ()
{
...
void
MLNodeLaplacian_FD::checkPoint (std::string const& file_name) const
{
...
#if (AMREX_SPACEDIM == 2)
void
MLNodeLaplacian_FD::fillIJMatrix (MFIter const& mfi, Array4<HypreNodeLap::Int const> const& nid,
