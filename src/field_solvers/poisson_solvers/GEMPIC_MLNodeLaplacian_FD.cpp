/*------------------------------------------------------------------------------
 file copied from the AMReX library to introduce slightly modified class of
 NodeLaplacian solver bsed on Finite Differences.
 Original AMReX file: AMReX_MLNodeLaplacian.cpp from commit 2d2d0c0
------------------------------------------------------------------------------*/
...
#include <GEMPIC_MLNodeLap_FD_K.H>
#include <GEMPIC_MLNodeLaplacian_FD.H>

namespace amrex
{
MLNodeLaplacian_FD::MLNodeLaplacian_FD(const Vector<Geometry> &a_geom,
...
MLNodeLaplacian_FD::~MLNodeLaplacian_FD() {}

void MLNodeLaplacian_FD::define(const Vector<Geometry> &a_geom, const Vector<BoxArray> &a_grids,
                                const Vector<DistributionMapping> &a_dmap, const LPInfo &a_info,
                                const Vector<FabFactory<FArrayBox> const *> &a_factory)
{
    ...
    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev)
    {
        ...
        const int mglev = 0;
        const int idim = 0;
        m_sigma[amrlev][mglev][idim].reset(new MultiFab(
            m_grids[amrlev][mglev], m_dmap[amrlev][mglev], 1, 1, MFInfo(), *m_factory[amrlev][0]));
    ...
    {
#ifdef AMREX_USE_EB
        m_integral[amrlev].reset(new MultiFab(m_grids[amrlev][0], m_dmap[amrlev][0], ncomp_i, 1,
...
void MLNodeLaplacian_FD::unimposeNeumannBC(int amrlev, MultiFab &rhs) const
{
...
void MLNodeLaplacian_FD::setSigma(int amrlev, const MultiFab &a_sigma)
{
    MultiFab::Copy(*m_sigma[amrlev][0][0], a_sigma, 0, 0, 1, 0);
}

void MLNodeLaplacian_FD::compDivergence(const Vector<MultiFab *> &rhs,
                                        const Vector<MultiFab *> &vel)
{
    compRHS(rhs, vel, Vector<const MultiFab *>(), Vector<MultiFab *>());
}

void MLNodeLaplacian_FD::compRHS(const Vector<MultiFab *> &rhs, const Vector<MultiFab *> &vel,
                                 const Vector<const MultiFab *> &rhnd,
                                 const Vector<MultiFab *> &a_rhcc)
{
    BL_PROFILE("MLNodeLaplacian_FD::compRHS()");
    ...
    for (int ilev = 0; ilev < m_num_amr_levels; ++ilev)
    {
        ...
        {
            rhcc[ilev].reset(
                new MultiFab(a_rhcc[ilev]->boxArray(), a_rhcc[ilev]->DistributionMap(), 1, 1));
            rhcc[ilev]->setVal(0.0);
            MultiFab::Copy(*rhcc[ilev], *a_rhcc[ilev], 0, 0, 1, 0);
            rhcc[ilev]->FillBoundary(geom.periodicity());

            rhs_cc[ilev].reset(
                new MultiFab(rhs[ilev]->boxArray(), rhs[ilev]->DistributionMap(), 1, 0));
        }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(*rhs[ilev], mfi_info); mfi.isValid(); ++mfi)
        {
            ...
            if (factory)
            {
                ...
                {
                    AMREX_HOST_DEVICE_PARALLEL_FOR_3D(bx, i, j, k, { rhsarr(i, j, k) = 0.0; });
                }
                else if (typ == FabType::singlevalued)
                {
                    ...
                    AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                        mlndlap_divu_eb(i, j, k, rhsarr, velarr, vfracarr, intgarr, dmskarr,
                                        dxinvarr);
                    });
                }
                else
            ...
            if (rhcc[ilev])
            {
                ...
                if (typ == FabType::singlevalued)
                {
                    ...
                    AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                        rhs_cc_a(i, j, k) =
                            mlndlap_rhcc_eb(i, j, k, rhccarr, vfracarr, intgarr, dmskarr);
                    });
                }
                else if (typ == FabType::covered)
                {
                    AMREX_HOST_DEVICE_PARALLEL_FOR_3D(bx, i, j, k, { rhs_cc_a(i, j, k) = 0.0; });
                }
                else
#endif
                {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (int ilev = m_num_amr_levels - 2; ilev >= 0; --ilev)
    {
        ...
        bool neumann_doubling =
            true;  // yes even for RAP, because unimposeNeumannBC will be called on rhs
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(*rhs[ilev]); mfi.isValid(); ++mfi)
        {
            if (has_fine_bndry[mfi])
            {
                ...
                Array4<Real const> rhccarr =
                    (rhcc[ilev]) ? rhcc[ilev]->const_array(mfi) : Array4<Real const>{};

                AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                    mlndlap_divu_cf_contrib(i, j, k, rhsarr, velarr, crhsarr, rhccarr, cdmskarr,
                                            ndmskarr, ccmskarr,
#if (AMREX_SPACEDIM == 2)
                                            is_rz,
#endif
...
void MLNodeLaplacian_FD::updateVelocity(const Vector<MultiFab *> &vel,
                                        const Vector<MultiFab const *> &sol) const
{
#if (AMREX_SPACEDIM == 2)
    bool is_rz = m_is_rz;
#endif
    ...
    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev)
    {
        ...
        for (MFIter mfi(*vel[amrlev], TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            if (factory)
            {
                ...
                {
                    AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                        mlndlap_mknewu_eb(i, j, k, varr, solarr, sigmaarr, vfracarr, intgarr,
                                          dxinv);
                    });
                }
                else
...
void MLNodeLaplacian_FD::getFluxes(const Vector<MultiFab *> &a_flux,
    ...
    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev)
    {
        ...
        for (MFIter mfi(sigma, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            if (factory)
            {
                ...
                {
                    AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                        mlndlap_mknewu_eb(i, j, k, farr, solarr, sigmaarr, vfracarr, intgarr,
                                          dxinv);
                    });
                }
                else
...
void MLNodeLaplacian_FD::averageDownCoeffs()
{
    BL_PROFILE("MLNodeLaplacian_FD::averageDownCoeffs()");
...
void MLNodeLaplacian_FD::averageDownCoeffsToCoarseAmrLevel(int flev)
{
    const int mglev = 0;
...
void MLNodeLaplacian_FD::averageDownCoeffsSameAmrLevel(int amrlev)
{
            ...
            for (MFIter mfi(*pcrse, TilingIfNotGPU()); mfi.isValid(); ++mfi)
            {
                ...
                if (idim == 0)
                {
                    AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                        bx, i, j, k, { mlndlap_avgdown_coeff_x(i, j, k, cfab, ffab); });
                }
                else if (idim == 1)
                {
#if (AMREX_SPACEDIM >= 2)
                    AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                        bx, i, j, k, { mlndlap_avgdown_coeff_y(i, j, k, cfab, ffab); });
#endif
                }
                else
                {
#if (AMREX_SPACEDIM == 3)
                    AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                        bx, i, j, k, { mlndlap_avgdown_coeff_z(i, j, k, cfab, ffab); });
#endif
                }
            }
...
void MLNodeLaplacian_FD::FillBoundaryCoeff(MultiFab &sigma, const Geometry &geom)
{
    BL_PROFILE("MLNodeLaplacian_FD::FillBoundaryCoeff()");
...
void MLNodeLaplacian_FD::buildStencil()
{
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(AMREX_SPACEDIM != 1,
                                     "MLNodeLaplacian_FD::buildStencil: 1d not supported");
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
        !m_geom[0][0].IsRZ(),
        "MLNodeLaplacian_FD::buildStencil: cylindrical not supported for RAP");
    ...
    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev)
    {
        for (int mglev = 0; mglev < m_num_mg_levels[amrlev]; ++mglev)
        {
            ...
            m_stencil[amrlev][mglev].reset(
                new MultiFab(amrex::convert(m_grids[amrlev][mglev], IntVect::TheNodeVector()),
                ...
                for (MFIter mfi(*m_stencil[amrlev][0], mfi_info); mfi.isValid(); ++mfi)
                {
                    ...
                    if (factory)
                    {
                        ...
                        else if (typ == FabType::singlevalued)
                        {
                            ...
                            AMREX_HOST_DEVICE_FOR_3D(ccbxg1, i, j, k, {
                                ...
                                {
                                    mlndlap_set_connection(i, j, k, cnarr, intgarr, vfracarr,
                                                           flagarr);
...
#endif
                    {
                        ...
                        AMREX_LAUNCH_HOST_DEVICE_LAMBDA(
                            bx, tbx, { mlndlap_set_stencil(tbx, starr, sgarr, dxinvarr); });
                    }
                }
            }
...
void MLNodeLaplacian_FD::fixUpResidualMask(int amrlev, iMultiFab &resmsk)
{
    if (!m_masks_built) buildMasks();
...
void MLNodeLaplacian_FD::prepareForSolve()
{
...
void MLNodeLaplacian_FD::restriction(int amrlev, int cmglev, MultiFab &crse, MultiFab &fine) const
{
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(*pcrse, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        else
        {
            ...
            AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                bx, i, j, k, { mlndlap_restriction_rap(i, j, k, cfab, ffab, stfab, mfab); });
        }
    }
...
void MLNodeLaplacian_FD::interpolation(int amrlev, int fmglev, MultiFab &fine,
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(fine, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        if (m_coarsening_strategy == CoarseningStrategy::RAP)
        {
            Array4<Real const> const &stfab = stencil->const_array(mfi);
            AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                bx, i, j, k, { mlndlap_interpadd_rap(i, j, k, ffab, cfab, stfab, mfab); });
        }
        ...
        else
        {
            ...
            AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                bx, i, j, k, { mlndlap_interpadd_aa(i, j, k, ffab, cfab, sfab, mfab); });
        }
    }
}

void MLNodeLaplacian_FD::averageDownSolutionRHS(int camrlev, MultiFab &crse_sol, MultiFab &crse_rhs,
...
void MLNodeLaplacian_FD::restrictInteriorNodes(int camrlev, MultiFab &crhs, MultiFab &a_frhs) const
{
...
void MLNodeLaplacian_FD::Fapply(int amrlev, int mglev, MultiFab &out, const MultiFab &in) const
{
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(out, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        if (m_coarsening_strategy == CoarseningStrategy::RAP)
        {
            ...
            AMREX_HOST_DEVICE_PARALLEL_FOR_3D(
                bx, i, j, k, { mlndlap_adotx_sten(i, j, k, yarr, xarr, stenarr, dmskarr); });
        }
...
void MLNodeLaplacian_FD::Fsmooth(int amrlev, int mglev, MultiFab &sol, const MultiFab &rhs) const
{
    ...
    if (Gpu::inLaunchRegion())
    {
        ...
        else if (m_use_harmonic_average && mglev > 0)
        {
            for (MFIter mfi(sol); mfi.isValid(); ++mfi)
            {
                ...
                {
                    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                        mlndlap_adotx_ha(i, j, k, Axarr, solarr, AMREX_D_DECL(sxarr, syarr, szarr),
                                         dmskarr,
#if (AMREX_SPACEDIM == 2)
                                         is_rz,
#endif
                    ...
                    AMREX_LAUNCH_DEVICE_LAMBDA(bx, tbx, {
                        mlndlap_jacobi_ha(tbx, solarr, Axarr, rhsarr,
                                          AMREX_D_DECL(sxarr, syarr, szarr), dmskarr, dxinvarr);
                    });
                }
            }
        }
        else
        {
            for (MFIter mfi(sol); mfi.isValid(); ++mfi)
            {
                ...
                for (int ns = 0; ns < nsweeps; ++ns)
                {
                    ...
                    AMREX_LAUNCH_DEVICE_LAMBDA(bx, tbx, {
                        mlndlap_jacobi_aa(tbx, solarr, Axarr, rhsarr, sarr, dmskarr, dxinvarr);
                    });
                }
            }
        }
...
#pragma omp parallel
                ...
                for (MFIter mfi(sol); mfi.isValid(); ++mfi)
                {
                    ...
                    for (int ns = 0; ns < nsweeps; ++ns)
                    {
                        mlndlap_gauss_seidel_aa(bx, solarr, rhsarr, sarr, dmskarr, dxinvarr
#if (AMREX_SPACEDIM == 2)
                                                ,
                                                is_rz
#endif
...
#pragma omp parallel
                ...
                for (MFIter mfi(sol, true); mfi.isValid(); ++mfi)
                {
                    ...
                    mlndlap_jacobi_aa(bx, solarr, Axarr, rhsarr, sarr, dmskarr, dxinvarr);
                }
            }
        }
    }
...
void MLNodeLaplacian_FD::normalize(int amrlev, int mglev, MultiFab &mf) const
{
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(mf, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        if (m_coarsening_strategy == CoarseningStrategy::RAP)
        {
            Array4<Real const> const &stenarr = stencil->const_array(mfi);
            AMREX_LAUNCH_HOST_DEVICE_LAMBDA(
                bx, tbx, { mlndlap_normalize_sten(tbx, arr, stenarr, dmskarr, s0_norm0); });
        }
        else if (m_use_harmonic_average && mglev > 0)
        {
            AMREX_D_TERM(Array4<Real const> const &sxarr = sigma[0]->const_array(mfi);
        ...
        else
        {
            ...
            AMREX_LAUNCH_HOST_DEVICE_LAMBDA(
                bx, tbx, { mlndlap_normalize_aa(tbx, arr, sarr, dmskarr, dxinv); });
        }
    }
}

void MLNodeLaplacian_FD::compSyncResidualCoarse(MultiFab &sync_resid, const MultiFab &a_phi,
                                                const MultiFab &vold, const MultiFab *rhcc,
                                                const BoxArray &fine_grids,
                                                const IntVect &ref_ratio)
{
    BL_PROFILE("MLNodeLaplacian_FD::SyncResCrse()");
            ...
            if (typ != FabType::covered)
            {
                ...
                if (has_fine)
                {
                    ...
                    AMREX_HOST_DEVICE_FOR_3D(ccbxg1, i, j, k, {
                        ...
                        {
                            AMREX_D_TERM(uarr(i, j, k, 0) = 0.0;, uarr(i, j, k, 1) = 0.0;
                                         , uarr(i, j, k, 2) = 0.0;);
                        }
                    });
                    ...
                    if (typ == FabType::singlevalued)
                    {
                        ...
                        Array4<Real const> const &intgarr = intg->const_array(mfi);
                        AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                            mlndlap_divu_eb(i, j, k, rhsarr, uarr, vfracarr, intgarr, dmskarr,
                                            dxinv);
                        });
                    }
                    else
                    ...
                    if (typ == FabType::singlevalued)
                    {
                        ...
                        AMREX_HOST_DEVICE_PARALLEL_FOR_3D(bx, i, j, k, {
                            ...
                            mlndlap_crse_resid(i, j, k, sync_resid_a, rhsarr, cccmsk, nddom, lobc,
                                               hibc, neumann_doubling);
                        });
                    }
                    else
#endif
...
void MLNodeLaplacian_FD::compSyncResidualFine(MultiFab &sync_resid, const MultiFab &phi,
                                              const MultiFab &vold, const MultiFab *rhcc)
{
    BL_PROFILE("MLNodeLaplacian_FD::SyncResFine()");
        ...
        for (MFIter mfi(sync_resid, mfi_info); mfi.isValid(); ++mfi)
        {
            ...
            if (typ != FabType::covered)
            {
                ...
                if (typ == FabType::singlevalued)
                {
                    ...
                    Array4<Real const> const &intgarr = intg->const_array(mfi);
                    AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                        mlndlap_divu_eb(i, j, k, rhsarr, uarr, vfracarr, intgarr, tmpmaskarr,
                                        dxinv);
                    });
                }
                else
                ...
                if (rhcc)
                {
                    ...
                    if (typ == FabType::singlevalued)
                    {
                        ...
                        AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                            Real rhs2 =
                                mlndlap_rhcc_eb(i, j, k, rhccarr, vfracarr, intgarr, tmpmaskarr);
                            rhsarr(i, j, k) += rhs2;
                        });
                    }
                    else
...
void MLNodeLaplacian_FD::reflux(int crse_amrlev, MultiFab &res, const MultiFab &crse_sol,
                                const MultiFab &crse_rhs, MultiFab &fine_res, MultiFab &fine_sol,
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(res, mfi_info); mfi.isValid(); ++mfi)
    {
        if ((*has_fine_bndry)[mfi])
        {
            ...
            AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                mlndlap_res_cf_contrib(i, j, k, resarr, csolarr, crhsarr, csigarr, cdmskarr,
                                       ndmskarr, ccmskarr, fcocarr, cdxinv, c_nd_domain,
#if (AMREX_SPACEDIM == 2)
                                       is_rz,
#endif
...
#ifdef AMREX_USE_EB
void MLNodeLaplacian_FD::buildIntegral()
{
    if (m_integral_built) return;

    BL_PROFILE("MLNodeLaplacian_FD::buildIntegral()");
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
            ...
            for (MFIter mfi(*intg, mfi_info); mfi.isValid(); ++mfi)
            {
                ...
                else
                {
                    ...
                    AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k, {
                        mlndlap_set_integral_eb(i, j, k, garr, flagarr, vfracarr, axarr, ayarr,
                                                bcarr);
                    });
                }
            }
...
void MLNodeLaplacian_FD::checkPoint(std::string const &file_name) const
{
    if (ParallelContext::IOProcessorSub())
    {
...
#else
            HeaderFile << "lobc = " << static_cast<int>(m_lobc[0][0]) << " "
                       << static_cast<int>(m_lobc[0][1]) << " " << static_cast<int>(m_lobc[0][2])
                       << "\n";
#endif
#if (AMREX_SPACEDIM == 1)
...
#else
            HeaderFile << "hibc = " << static_cast<int>(m_hibc[0][0]) << " "
                       << static_cast<int>(m_hibc[0][1]) << " " << static_cast<int>(m_hibc[0][2])
                       << "\n";
#endif
            // m_coarse_data_for_bc: not used
            HeaderFile << "maxorder = " << getMaxOrder() << "\n";

            // MLNodeLaplacian_FD stuff
        ...
        for (int ilev = 0; ilev < NAMRLevels(); ++ilev)
        {
            ...
            m_grids[ilev][0].writeOn(HeaderFile);
            HeaderFile << "\n";
        }
    }
...
#if (AMREX_SPACEDIM == 2)
void MLNodeLaplacian_FD::fillIJMatrix(MFIter const &mfi, Array4<HypreNodeLap::Int const> const &nid,
                                      Array4<int const> const &owner,
                                      Vector<HypreNodeLap::Int> &ncols,
                                      Vector<HypreNodeLap::Int> &rows,
