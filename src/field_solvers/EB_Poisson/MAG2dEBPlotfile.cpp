...
void
MAG2dEB::writeEBPlotfileS() const
{
  if(polprint) {
    const int ncomp = 4;
    Vector<std::string> varname = {"solution", "rhs", "exact_solution", "error"};
    
    MultiFab plotmf(grids, dmap, ncomp, 0, MFInfo(), *ebf);
    ...
    EB_WriteSingleLevelPlotfile("ploteb", plotmf,
                            varname, geom, 0.0, 0 );
  }
}
...
/*
void
MAG2dEB::writeEBPlotfileM() const
{
    const int ncomp = 4;
    Vector<std::string> varname = {"solution", "rhs", "exact_solution", "error"};
    ...
    EB_WriteMultiLevelPlotfile("plot", nlevels,
