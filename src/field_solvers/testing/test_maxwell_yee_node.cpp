...
void init_E_B(amrex::IntVect  lo,
  ...
  for(int k=lo[2]; k<=hi[2]; k++){
    z = prob_lo[2] + (double)k * dx[2]; //node
    for(int j=lo[1]; j<=hi[1]; j++){
      y = prob_lo[1] + (double)j * dx[1]; //node
      for(int i=lo[0]; i<=hi[0]; i++){
        x = prob_lo[0] + (double)i * dx[0]; //node
	
	// the box for these values:
	IndexType typ({1,1,1}); //node
	Box nc({i,j,k}, {i+1,j+1,k+1}, typ); //node
	
        E_new.setVal(cos(x+y+z), nc, 0, 1); //node
	E_new.setVal(-2*cos(x+y+z), nc, 1, 1); //node
	E_new.setVal(cos(x+y+z), nc, 2, 1); //node

	// shift all by dx/2 to get node-centered
	B_new.setVal(sqrt(3)*cos(x+y+z), nc, 0, 1); //node
	B_new.setVal(0, nc, 1, 1); //node
	B_new.setVal(-sqrt(3)*cos(x+y+z), nc, 2, 1); //node
...
void update_E_B(amrex::IntVect lo,
  ...
  for(int k=lo[2]; k<=hi[2]; k++){
	...
	B_old.getVal(c5, {  i,  j,  k},1,1);
        E_new.setVal(*c1+dt*((*c2-*c3)/dx[1]-(*c4-*c5)/dx[2]), nc, 0, 1); //node
	...
	B_old.getVal(c5, {  i,  j,  k},2,1);
        E_new.setVal(*c1+dt*((*c2-*c3)/dx[2]-(*c4-*c5)/dx[0]), nc, 1, 1); //node
	...
	B_old.getVal(c5, {  i,  j,  k},0,1);
        E_new.setVal(*c1+dt*((*c2-*c3)/dx[0]-(*c4-*c5)/dx[1]), nc, 2, 1); //node
	...
	E_old.getVal(c5, {  i,  j,  k},1,1);
        B_new.setVal(*c1-dt*((*c2-*c3)/dx[1]-(*c4-*c5)/dx[2]), nc, 0, 1); //node
	...
	E_old.getVal(c5, {  i,  j,  k},2,1);
        B_new.setVal(*c1-dt*((*c2-*c3)/dx[2]-(*c4-*c5)/dx[0]), nc, 1, 1); //node
	...
	E_old.getVal(c5, {  i,  j,  k},0,1);
        B_new.setVal(*c1-dt*((*c2-*c3)/dx[0]-(*c4-*c5)/dx[1]), nc, 2, 1); //node

        // Analytic values
	E_sol.setVal(cos(x+y+z-sqrt(3)*time), nc, 0, 1); //node
	E_sol.setVal(-2*cos(x+y+z), nc, 1, 1); //node
	E_sol.setVal(cos(x+y+z-sqrt(3)*time), nc, 2, 1); //node

	// shift all by dx/2 to get node-centered
	B_sol.setVal(sqrt(3)*cos(x+y+z-sqrt(3)*time), nc, 0, 1); //node
	B_sol.setVal(0, nc, 1, 1); //node
	B_sol.setVal(-sqrt(3)*cos(x+y+z-sqrt(3)*time), nc, 2, 1); //node
...
//------------------------------------------------------------------------------
  ...
  Geometry geom;
  {
    ...
    IndexType typ({1,1,1}); //node
    Box domain(dom_lo, dom_hi, typ); //node

    // individual boxes (boxarray)
    ba.define(domain); //node
    ba.convert(typ); //node
    
    // Break up boxarray "ba" into chunks no larger than "max_grid_size" along a direction
    ba.maxSize(max_grid_size); //node
