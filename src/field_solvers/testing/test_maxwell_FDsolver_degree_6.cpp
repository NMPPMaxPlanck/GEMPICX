...
std::tuple<amrex::Real, amrex::Real> maxwell (const int n)
{
    ...
    {
        // dError += maxErrorMidpoint<hodgeDegree>(geom, funcD[comp], D.data[comp], params.dr(), 2,
        // true, comp, Nt*dt);
...
int main (int argc, char *argv[])
{
    amrex::Initialize(argc, argv);
    {
        ...
        amrex::PrintToFile("test_maxwell_FDsolver_degree_6.output")
            << GEMPIC_SPACEDIM << "D Maxwell degree 6 convergence test:" << std::endl;
        amrex::PrintToFile("test_maxwell_FDsolver_degree_6.output") << std::endl;
        amrex::PrintToFile("test_maxwell_FDsolver_degree_6.output").SetPrecision(3)
        ...
        {
            std::rename("test_maxwell_FDsolver_degree_6.output.0",
                        "test_maxwell_FDsolver_degree_6.output");
        }
    }
    amrex::Finalize();
}
