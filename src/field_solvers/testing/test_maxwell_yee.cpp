...
void main_main()
{
    ...
    amrex::Print() << "x DIM: " << GEMPIC_SPACEDIM << ", v&E DIM: " << vdim << ", B DIM: " << bdim
                   << std::endl;
    ...
    PrintToFile("test_maxwell_yee.output").SetPrecision(5)
        << "Bx error: " << E_B_error[3] << " |By error: " << E_B_error[4]
        << " |Bz error: " << E_B_error[5] << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    if (ParallelDescriptor::MyProc() == 0)
        std::rename("test_maxwell_yee.output.0", "test_maxwell_yee.output");
