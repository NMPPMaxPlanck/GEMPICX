...
{  //------------------------------------------------------------------------------
    ...
    fields_E[0] = "cos(x+y+z-sqrt(3.0)*t)"; //Ex
    fields_E[1] = "-2*cos(x+y+z-sqrt(3.0)*t)"; //Ey
    fields_E[2] = "cos(x+y+z-sqrt(3.0)*t)"; //Ez
    ...
    std::array<int,GEMPIC_SPACEDIM> n_cell = {AMREX_D_DECL(128,128,128)}; // spatial discretization: number of cells in each direction, currently: 128x128x128
    int numstep = 10; // number of timesteps
    ...
    for (int n=1;n<=mw_yee.nsteps;n++){
        ...
        mw_yee.advance_E(infra, VlMa.dt, true, false, &(mw_yee.HB_Array), &(mw_yee.E_Array)); // we apply curl to B and set E = E+dt*curl(B)
