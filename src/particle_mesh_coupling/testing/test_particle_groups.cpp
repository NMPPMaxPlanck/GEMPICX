...
AMREX_GPU_HOST_DEVICE amrex::Real funct_Bz(amrex::Real x, amrex::Real y, amrex::Real z,
                                           amrex::Real t)
{
...
void main_main()
{
    ...
    amrex::ParallelDescriptor::ReduceRealSum(mass, amrex::ParallelDescriptor::IOProcessorNumber());
    ...
    for (int cmp = 0; cmp < vdim; cmp++)
    {
        ...
        amrex::ParallelDescriptor::ReduceRealSum(mom_tmp,
    ...
    switch (vdim)
    {
        case 1:
            ...
            break;
        case 2:
            PrintToFile("test_particle_groups_additional.tmp")
                << momentum[0] << " " << momentum[1] << std::endl;
            break;
        case 3:
            PrintToFile("test_particle_groups_additional.tmp")
                << momentum[0] << " " << momentum[1] << " " << momentum[2] << std::endl;
            break;
    }
    ...
    switch (vdim)
    {
        ...
        case 2:
            PrintToFile("test_particle_groups_additional.tmp")
                << kinetic_energy[0] << " " << kinetic_energy[1] << std::endl;
            break;
        case 3:
            PrintToFile("test_particle_groups_additional.tmp")
                << kinetic_energy[0] << " " << kinetic_energy[1] << " " << kinetic_energy[2]
                << std::endl;
            break;
    }
}
...
int main(int argc, char* argv[])
{
    ...
    if (ParallelDescriptor::MyProc() == 0)
        std::rename("test_particle_groups_additional.tmp.0",
                    "test_particle_groups_additional.output");
    amrex::Finalize();
}
