    ...
    TEST_F(DepositRhoTest, SingleParticleOnNode) {
        ...
        for (amrex::ParIter<0, 0, vdim + 1, 0> pti(*particleGroup[0], 0); pti.isValid(); ++pti)
        {
            ...
            checkRho(__LINE__, rho[pti].array(), infra.n_cell.dim3(),
                    ...
                    {[] (AMREX_D_DECL(int a, int b, int c)) {return GEMPIC_D_COND(a == 0,
                                                                               && b == 0,
                                                                               && c == 0);}},
    ...
    TEST_F(DepositRhoTest, SingleParticleMiddle) {
        ...
        for (amrex::ParIter<0, 0, vdim + 1, 0> pti(*particleGroup[0], 0); pti.isValid(); ++pti)
        {
            ...
            checkRho(__LINE__, rho[pti].array(), infra.n_cell.dim3(),
                    ...
                    {[] (AMREX_D_DECL(int a, int b, int c)) {return GEMPIC_D_COND(a >= 9,
                                                                               && b >= 9,
                                                                               && c >= 9);}},
        ...
        for (amrex::ParIter<0, 0, vdim + 1, 0> pti(*particleGroup[0], 0); pti.isValid(); ++pti)
        {
            ...
            checkRho(__LINE__, rho[pti].array(), infra.n_cell.dim3(),
                ...
                {[] (AMREX_D_DECL(int a, int b, int c)) {return GEMPIC_D_COND(a == 0,
                                                                           && b == 0,
                                                                           && c == 0);},
                [] (AMREX_D_DECL(int a, int b, int c)) {return GEMPIC_D_COND(a == 1,
                                                                          && b == 1,
                                                                          && c == 1);},
                [] (AMREX_D_DECL(int a, int b, int c)) {return GEMPIC_D_COND(a, +b, +c) == 1;},
                [](AMREX_D_DECL(int a, int b, int c)){return (GEMPIC_D_COND(a*a,+b*b,+c*c) == 2);}},
        ...
        for (amrex::ParIter<0, 0, vdim + 1, 0> pti(*particleGroup[0], 0); pti.isValid(); ++pti)
        {
            ...
            checkRho(__LINE__, rho[pti].array(), infra.n_cell.dim3(),
                {[](AMREX_D_DECL(int a, int b, int c)){return GEMPIC_D_COND(a == 0,
                                                                         && b == 0,
                                                                         && c == 0);},
                [](AMREX_D_DECL(int a, int b, int c)){return GEMPIC_D_COND((a == 5 || a == 6),
                                                                        && (b == 5 || b == 6),
                                                                        && (c == 5 || c == 6));}},
