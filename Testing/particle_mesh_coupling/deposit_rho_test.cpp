                              ...
                              amrex::Real charge)
{
    ...
    amrex::ParallelFor(np,
                       [=] AMREX_GPU_DEVICE(long pp)
                       {
                           ...
                           Spline::SplineBase<degX, degY, degZ> spline(position, infra.m_plo,
                                                                       infra.m_dxi);
...
protected:
    ...
    const int m_nghost{GEMPIC_TestUtils::init_n_ghost(s_degX, s_degY, s_degZ)};
    ...
    void SetUp () override
    {
        ...
        m_deRham = std::make_shared<GEMPIC_FDDeRhamComplex::FDDeRhamComplex>(m_infra, hodgeDegree,
                                                                             maxSplineDegree);
...
TEST_F(DepositRhoTest, NullTest)
{
    ...
    for (amrex::ParIter<0, 0, s_vDim + 1, 0> pti(*m_particleGroup[0], 0); pti.isValid(); ++pti)
    {
        ...
        EXPECT_EQ(numParticles,
                  np);  // Only one particle added by GEMPIC_TestUtils::addSingleParticles
        ...
        EXPECT_EQ(
            1,
            weight[0]);  // weight correctly transferred from GEMPIC_TestUtils::addSingleParticles
...
TEST_F(DepositRhoTest, DoubleParticleMultipleSpecies)
{
    ...
    GEMPIC_TestUtils::add_single_particles(m_particleGroup, m_infra, pWeights, pPos, pSpec);
    GEMPIC_TestUtils::add_single_particles(m_particleGroup, m_infra, eWeights, ePos, eSpec);
